 ...
 public:
  explicit UbloxFirmware8(const std::string & frame_id, std::shared_ptr<diagnostic_updater::Updater> updater, std::shared_ptr<FixDiagnostic> freq_diag, std::shared_ptr<Gnss> gnss, rclcpp::Node* node)
    ...
    if (getRosBoolean(node_, "publish.rxm.raw")) {
      RCLCPP_INFO(node_->get_logger(), "Enabling RXM-RAWX topic: rxmraw");
      rxm_rawx_pub_ = node->create_publisher<ublox_msgs::msg::RxmRAWX>("rxmraw", 1);
    }
    if (getRosBoolean(node_, "publish.rxm.sfrb")) {
      RCLCPP_INFO(node_->get_logger(), "Enabling RXM-SFRBX topic: rxmsfrb");
      rxm_sfrbx_pub_ = node->create_publisher<ublox_msgs::msg::RxmSFRBX>("rxmsfrb", 1);
    }
  }
